 ...
 */
  ...
  if (--sched_ticks == uptodate_quantum * SCHED_RATE) {
 ...
 */
  ...
  if (--sched_ticks == 0) {
	...
	if(rdy_head[USER_Q]->group == GROUP_A) {
	   sched_ticks = time_quantum/100 * SCHED_RATE;
	   }
	   else {
	   sched_ticks = (100-time_quantum)/100* SCHED_RATE;
	   }
	prev_ptr = bill_ptr;		/* new previous process */
  }
