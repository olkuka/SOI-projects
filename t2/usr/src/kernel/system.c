...
FORWARD _PROTOTYPE( int do_changegroup, (message *m_ptr)  );
...
/* Main entry point of sys_task.  Get the message and dispatch on type. */
  ...
  while (TRUE) {
	...
	switch (m.m_type) {	/* which system call */
	    ...
	    case SYS_CHANGEGROUP:  r = do_changegroup(&m);  break;
...
message *m_ptr;
{
   time_quantum = m_ptr->m1_i1; 
   return 0;
}

PRIVATE int do_changegroup(m_ptr)
message *m_ptr;
{
