...
int do_hole_map() {
	...
	phys_clicks size = mm_in.m1_i1/(sizeof(phys_clicks)*2);
	unsigned int i = 0;

	while (i < size && hp != NIL_HOLE && hp->h_base < swap_base) {
		buffer[i] = hp->h_len;		
		i++;
		buffer[i] = hp->h_base;			
		i++;
